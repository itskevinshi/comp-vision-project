#!/usr/bin/env python3
"""
Use the finetuned weather-robust model for inference.
"""
#########################################################
...
if __name__ == "__main__":
    ...
    # -----------------------------------------------------
    # 4.1 Load the Finetuned Model
    # -----------------------------------------------------
    # Path to your finetuned .pth file
    model_path = "src/models/finetuned/resnet50_weather_robust.pth"
    

    # Create a resnet50 and manually set the final fc to 38 outputs
    finetuned_model = models.resnet50(pretrained=False)
    num_features = finetuned_model.fc.in_features
    finetuned_model.fc = nn.Linear(num_features, 38)  # 38 classes

    # Load the finetuned checkpoint
    loaded_state = torch.load(model_path, map_location=device)
    finetuned_model.load_state_dict(loaded_state)
    ...
    # Evaluate
    rain_results = evaluate(finetuned_model, rain_test_loader)
    ...
    raw_results = evaluate(finetuned_model, raw_test_loader)
    ...
    print("\nDone! Finetuned model evaluation on augmented images complete.")